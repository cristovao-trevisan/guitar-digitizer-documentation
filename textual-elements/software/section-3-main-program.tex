...
At first let's take a look at the resources:
\begin{itemlist}
...
So far this document has not said a word about how to model the resources, state or
functionality as objects, and it won't ever, because it does not have any classes,
...
As the Redux store keeps the all usable state (\autoref{reducers-table}) it is also
...
The entry point for this project is both a declarator and connector. It allocates
...
The amplitude calculation is a simple absolute average removing the mean. The
...
listed at \autoref{test-list}, so to give an idea of how they work.
